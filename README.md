...
- **進捗リセット**: アプリ内の設定画面からすべての学習記録をリセットし、最初からやり直すことができます。このボタンは現在、設定画面では非表示になっています。
- **効果音と触覚フィードバック**: 正解・不正解時およびステージクリア時に効果音と触覚フィードバックが提供され、設定でON/OFFを切り替えられます。
...
*   **2. 進行状況管理（ProgressStore）**
    *   ステージクリア状態が正しく記録・復元される (強制終了後も保持されることを確認済み)
        *   ステージクリア → 再起動後も保持されるか
...
*   **4. 音・触覚フィードバック**
    *   正答・誤答時およびステージクリア時の効果音が正しく鳴る（ミュート設定時は鳴らないことを確認済み）
    *   触覚フィードバックが正しく動作する
